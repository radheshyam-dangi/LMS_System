...
export class LearningPathOwnerGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const pathId =
      params.id || params.pathId || body.learningPathId || query.learningPathId;
    const moduleId =
      params.id || params.moduleId || body.moduleId || query.moduleId;
    const lessonId =
      params.id || params.lessonId || body.lessonId || query.lessonId;

    // 3. RESOLVE OWNER ID ACCORDING TO ENTITY TYPE & ROUTE CONTEXT
    if (
      pathId &&
      (request.baseUrl.includes('learning-paths') || body.learningPathId)
    ) {
      const path = await this.dataSource
        .getRepository(LearningPathEntity)
        .findOne({
          where: { id: pathId },
          relations: ['createdBy'],
        });
      ...
      if (assignment) {
        targetPathOwnerId =
          assignment.lesson?.module?.learningPath?.createdBy?.id ||
          (assignment.lesson?.module?.learningPath as any)?.createdById;
      }
    }
