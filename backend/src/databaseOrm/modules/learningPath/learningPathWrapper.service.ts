...
export class LearningPathService {
  ...
  async getPathsForUserContext(
    ...
    if (role === 'Trainee') {
      return allPaths.filter((path) =>
        path.assignedToTraineeIds?.includes(userId),
      );
    }
  ...
  async assignPathToTrainee(
    ...
    return await this.learningPathEntityService.assignTraineeToPath(
      pathId,
      traineeId,
    );
  }
  ...
  async getPathById(pathId: string): Promise<LearningPathEntity> {
    const path =
      await this.learningPathEntityService.findPathWithDetails(pathId);
    if (!path) {
