  ...
  async createResource(dto: {
    ...
    if (!moduleId && !lessonId) {
      throw new BadRequestException(
        'Resource must be attached to either a Module or a Lesson.',
      );
    }
    ...
    if (moduleId) {
      moduleEntity =
        (await this.moduleRepo.findOne({ where: { id: moduleId } })) ??
        undefined;
    }

    if (lessonId) {
      lessonEntity =
        (await this.lessonRepo.findOne({ where: { id: lessonId } })) ??
        undefined;
    }
