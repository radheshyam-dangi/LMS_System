...
export class AnalyticsEntityService {
  ...
  async getDashboardStats(currentUser?: any) {
    ...
    const isTrainee = currentUser?.roles?.some(
      (r: any) => String(r.name || r).toLowerCase() === 'trainee',
    );
    const isTrainer = currentUser?.roles?.some(
    ...
    const completionRate =
      totalAssignments > 0
        ? Math.min(
            Math.round(
              (evaluatedSubs.filter(
                (s) => s.status === 'Accepted' || s.status === 'Evaluated',
              ).length /
                totalAssignments) *
                100,
            ),
    ...
    const skillDistribution = [...skillCounts.entries()].map(
      ([name, count]) => ({
        name,
        count,
    ...
    const moduleCompletion = modules.map((mod) => {
      ...
      const moduleSubs = allSubmissions.filter(
        (s) =>
          s.assignment?.module?.id === mod.id ||
          (s.assignment as any)?.moduleId === mod.id,
      );
    ...
    // Calculate trainee progress details for Modal
    ...
    allSubmissions.forEach((s) => {
      ...
      const tName =
        (s.trainee as any)?.name || 'Trainee ' + String(tId).substring(0, 4);
      if (tId) {
    ...
    const traineeProgressList = Array.from(traineeStatsMap.entries()).map(
      ([name, stats]) => {
        const expected = totalAssignments > 0 ? totalAssignments : 10;
    ...
    const pathPerformance = paths.map((p) => {
      ...
      // Calculate trainee-specific lines for Trainer/Admin view
      const pathTrainees = new Map<string, number>();
      pathSubs.forEach((s) => {
        const tId = s.trainee?.id || (s as any).traineeId;
        const tName =
          (s.trainee as any)?.name || 'Trainee ' + String(tId).substring(0, 4);
        if (tId) {
          pathTrainees.set(tName, (pathTrainees.get(tName) || 0) + 1);
        }
      });

      const traineeSeries: any = {};
      pathTrainees.forEach((count, name) => {
        traineeSeries[name] = Math.min(Math.round((count / 5) * 100), 100); // mock calculation based on count
      });
    ...
    return {
      ...
      totalLessons,
      totalAssignments:
        trainerExpected > 0 ? trainerExpected : totalAssignments,
      pendingReviews,
      completionRate,
      averageScore,
      activeEnrollments,
      ...
      tasksCompleted:
        trainerSubmitted > 0
          ...
          : isTrainee
            ? new Set(
                allSubmissions
                  .map((s) => s.assignment?.id || (s as any).assignmentId)
                  .filter(Boolean),
              ).size
            : evaluatedSubs.length,
      learningVelocity: Math.round(evaluatedSubs.length / 4),
      currentStreak: 0,
      learningStyle: 'Visual',
      estimatedCompletionDate: 'N/A',
      trainingEffectiveness: Math.round(
        averageScore * 0.6 + completionRate * 0.4,
      ),
      totalGainedScore: averageScore,
      totalMaxScore: 100,
      skillGrowth: completionRate,
      charts: {
        progressTrends,
        weeklyScores,
        skillDistribution,
