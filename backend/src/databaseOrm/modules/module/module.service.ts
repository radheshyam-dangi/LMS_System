...
export class ModuleEntityService extends BaseService<ModuleEntity> {
  ...
  async checkIsPathOwner(
    ...
    const ownerId =
      learningPath.createdBy?.id || (learningPath as any)?.createdById;
  ...
  async createModuleForPath(
    ...
    if (!learningPathId) {
      throw new BadRequestException(
        'learningPathId is required to associate a module with a path.',
      );
    }
    ...
    try {
      ...
      if (!learningPath) {
        throw new NotFoundException(
          `Learning Path with ID "${learningPathId}" does not exist.`,
        );
      }
      ...
      if (!creator) {
        throw new BadRequestException(
          `User session invalid. User ID "${creatorId}" not found.`,
        );
      }
      ...
      const newModule = this.repository.create({
        ...
        durationLabel:
          durationLabel ??
          (durationWeeks ? `${durationWeeks} weeks` : '2 weeks'),
  ...
  async updateModule(id: string, dto: any): Promise<ModuleEntity> {
    ...
    if (dto.durationWeeks !== undefined) {
      ...
      if (!dto.durationLabel)
        patch.durationLabel = `${patch.durationWeeks} weeks`;
    }
    if (dto.keyPoints !== undefined) {
      ...
      await this.keyPointRepository.delete({ module: { id } });
