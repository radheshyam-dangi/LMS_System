...
export class LearningPathEntityService extends BaseService<LearningPathEntity> {
  ...
  constructor(
    ...
    this.enrollmentRepository = this.datasource.getRepository<EnrollmentEntity>(EnrollmentEntity);
  }
  ...
  async createPath(dto: any, creatorId: string): Promise<LearningPathEntity> {
    const { name, title, description, difficulty, duration, skillsTags, imageUrl, status } = dto;
    ...
    try {
      ...
      const newPath = this.repository.create({
        ...
        skillsTags: Array.isArray(skillsTags) ? skillsTags : skillsTags ? skillsTags.split(',') : ['General'],
      ...
      throw new InternalServerErrorException(`Failed to create learning path: ${error.message}`);
    }
  }
   ...
   */
  async assignTraineeToPath(pathId: string, traineeId: string): Promise<LearningPathEntity> {
