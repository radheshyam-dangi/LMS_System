...
export const curriculumService = {
  ...
  fetchModuleById: async (moduleId: string, token: string) => {
    const response = await axios.get(
  ...
  deleteModule: async (moduleId: string, token: string) => {
  ...
  deleteLesson: async (lessonId: string, token: string) => {
    const response = await axios.delete(
      `${API_BASE_URL}/lessons/${lessonId}`,
      getAuthHeaders(token)
    );
  ...
  createTask: async (
    payload: {
      title: string;
      instructions?: string;
      assignmentType?: 'Subjective' | 'MCQ' | 'External'; // Added External
