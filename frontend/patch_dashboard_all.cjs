...
// 4. Replace Admin/Trainer Charts
const adminChartsCode = `<section style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px', marginBottom: '24px' }}>
  <DashboardCharts
    ...
    type="progress"
    datasets={{
      daily: (charts.dailyProgressTrends || []).map((d) => ({ ...d, name: d.label || d.name })),
      weekly: (charts.weeklyProgressTrends || charts.progressTrends || []).map((d) => ({ ...d, name: d.label || d.name })),
      monthly: (charts.yearlyProgressTrends || []).map((d) => ({ ...d, name: d.label || d.name }))
    }}
  />
  <DashboardCharts
...
// 5. Inject components LAST so that the SVG replacements don't match the injected components!
content = content.replace(
